...
class Login(tk.Tk):
    ...
    def createWidget(self):
        ...
        input_email = ttk.Entry(self,width=50, font=('Arial', 14)).grid(column=0, row=2)

        ttk.Label(self, width=50,text='Password',foreground='#eeeeee',background='#181818', font=('Arial', 16)).grid(column=0, row=3,  pady=(10,0))
        input_password = ttk.Entry(self, width=50,show='*', font=('Arial', 14)).grid(column=0, row=4)

        self.image_singin = tk.PhotoImage(file='./assets/image/sign-in.png')
        button_font = font.Font(family='Arial', weight='bold', size=16)
        tk.Button(self, activebackground='#DD5555', background='#CC5555', width=200, borderwidth=0,font=button_font, text='Sign In ', image=self.image_singin, compound='right').grid(ipady=5,column=0, sticky=tk.E, row=5, pady=10)
